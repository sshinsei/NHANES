rm(list = ls())
setwd('E:/LZ')
if(! dir.exists("25070")) dir.create("25070")
setwd('./25070')
...
wp$LBXSAL <- wp$LBXSAL*10 # 白蛋白 g/dl转为g/L
wp_dat <- rename(.data=wp,
                 albumin = LBXSAL, # 白蛋白 g/l
...
ALQ_J <- as.data.frame(ALQ_J)  # 确保ALQ_J是数据框
ALQ_J$ALQ101 <- case_when(ALQ_J$ALQ101==0 ~ "No",
                          ALQ_J$ALQ101==77 ~ "Refused",
                          ALQ_J$ALQ101==99 ~ "Don't know",
                          TRUE ~ "Yes")
table(ALQ_J$ALQ101)
ALQ_J$ALQ101 <- as.numeric(recode(ALQ_J$ALQ101,No=2,Yes=1,Refused=7,`Don't know`=9))
ALQ_J <- rename(.data = ALQ_J, ALQ120Q = ALQ121)

ALQ <- rbind(ALQ_D[,c("SEQN","ALQ101","ALQ120Q","ALQ130")],
             ALQ_E[,c("SEQN","ALQ101","ALQ120Q","ALQ130")],
             ALQ_F[,c("SEQN","ALQ101","ALQ120Q","ALQ130")],
             ALQ_G[,c("SEQN","ALQ101","ALQ120Q","ALQ130")],
             ALQ_H[,c("SEQN","ALQ101","ALQ120Q","ALQ130")],
             ALQ_I[,c("SEQN","ALQ101","ALQ120Q","ALQ130")]
             #ALQ_J[,c("SEQN","ALQ101","ALQ120Q","ALQ130")]
) 

ALQ_dat <- rename(.data=ALQ,
                  Drink =ALQ101,freq = ALQ120Q,number=ALQ130
) 
...
BIOPRO <- rbind(
               BIOPRO_D[,c("SEQN","LBDSTRSI","LBXSTR","LBXSUA","LBDSBUSI","LBDSCRSI",
                           "LBDSCHSI","LBXSATSI","LBXSASSI","LBDSUASI","LBXSCR")],
               ...
               BIOPRO_I[,c("SEQN","LBDSTRSI","LBXSTR","LBXSUA","LBDSBUSI","LBDSCRSI",
                           "LBDSCHSI","LBXSATSI","LBXSASSI","LBDSUASI","LBXSCR")]
               #BIOPRO_J[,c("SEQN","LBDSTRSI","LBXSTR","LBXSUA","LBDSBUSI","LBDSCRSI","LBDSCHSI","LBXSATSI","LBXSASSI")]
) 
BIOPRO$LBDSCRSI <- BIOPRO$LBDSCRSI/1000 # 转为mmol/L
BIOPRO$LBDSUASI <- BIOPRO$LBDSUASI/1000 # 转为mmol/L

BIOPRO_dat <- rename(.data=BIOPRO,TG_mmol=LBDSTRSI,TG_mg=LBXSTR,
                     UA_mg=LBXSUA,UA_mmol=LBDSUASI,CR_mg=LBXSCR,
                      CR_mmol=LBDSCRSI,BUN_mmol=LBDSBUSI,CHL_mmol=LBDSCHSI,
                     ALT=LBXSATSI,AST=LBXSASSI)
