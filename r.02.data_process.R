...
dat$MI <- factor(dat$MI,levels = c(1,0),labels = c("Yes","No"))
table(dat$MI,useNA = "ifany")
...
dat$Hypertension <- ifelse(dat$hypter==1|dat$SBP_new>=140|dat$DBP_new>=90,1,2)
dat$Hypertension <- factor(dat$Hypertension,levels = c(1,2),labels = c("Yes","No"))
table(dat$Hypertension,useNA = "ifany")
...
# PIV 不显著
# ABSI = WC/(BMI²/³ × Height¹/²) 
# UHR (%) = (UA [mg/dL] ÷ HDL [mg/dL]) × 100 PMID:40241174 
# BUCR PMID:40082883 (BUN(mmol/L)*2.8*88.4)/CR(mmol/L)
...
dat$index <- (dat$UA_mg/dat$HDL_mg)*100
...
dat$Stroke <- ifelse(dat$Stroke==1,1,
                     ifelse(dat$Stroke == 7| dat$Stroke ==9, NA,0))
dat$Stroke <- factor(dat$Stroke,levels = c(1,0))
table(dat$Stroke,useNA = "ifany")
...
data_clean$wt <- data_clean$WTMEC2YR/6 # 2007-2016 7个周期
